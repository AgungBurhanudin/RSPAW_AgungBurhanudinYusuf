...
def get_token(db, api_host, api_username, api_password):
    api_url = f"{api_host}simpra/generate"
    ...
    # request
    print(headers)
    response = requests.request("GET", api_url, headers=headers, data={}, timeout=10, verify=False)
    ...
    if response.status_code == 200:
        ...
        pprint("status_code: " + str(response.status_code))
        raise HTTPException(status_code=500, detail="gagal generate token " + response.content.decode())
